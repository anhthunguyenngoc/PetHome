...
public class SalonServiceList extends ServiceList{
	...
	public void getlistAPI() throws Exception {
		...
		ArrayList<String> var = new ArrayList<String>(Arrays.asList("id", "process", "quantitative", "name", "introduction", "price"));	   				
		...
		if(stateCode == 200) {
			for(int j=0; j< res.size(); j++) {
				SalonService service = new SalonService(res.get(j).get(0), this.id, res.get(j).get(3), res.get(j).get(4), res.get(j).get(5), res.get(j).get(1), res.get(j).get(2));
				listAPI.add(service);
			}		
	...
	public void addService(ArrayList<String> value) throws Exception {
		SalonService service = new SalonService(this.id, value.get(2), value.get(3), value.get(4), value.get(0), value.get(1));
	}
